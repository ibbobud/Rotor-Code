...
class WelcomeBanner(Static):
    FLASH_COLOR = "#FFFFFF"
    TARGET_COLORS = ("#FFFFFF", "#E0E0E0", "#C8102E")
    BORDER_TARGET_COLOR = "#C8102E"
    ...
    def _initialize_static_line_suffixes(self) -> None:
        ...
        # Info lines appear below the logo (not beside it)
        self._static_info_line1 = f"[b]Rotor-Code v{__version__}[/]"
        self._static_info_line2 = f"[dim]{self.config.active_model} · {model_count} models · {mcp_count} MCP servers[/]"
        self._static_info_line3 = f"[dim]{self.config.effective_workdir}[/]"
        self._static_info_line4 = f"[dim]Type[/] [{self.BORDER_TARGET_COLOR}]/help[/] [dim]for more information[/]"
    ...
    def _init_after_styles(self) -> None:
        ...
        # Initialize static info lines (slots 4-7: blank, info1, info2, info3, info4)
        self._cached_text_lines[3] = Text("")  # blank line after logo
        self._cached_text_lines[4] = Text.from_markup(self._static_info_line1)
        self._cached_text_lines[5] = Text.from_markup(self._static_info_line2)
        self._cached_text_lines[6] = Text.from_markup(self._static_info_line3)
        self._cached_text_lines[7] = Text.from_markup(self._static_info_line4)
    ...
    def _update_display(self) -> None:
        # Update the 3 animated logo lines (slots 0-2)
        for idx in range(3):
    ...
    def _build_line(self, line_idx: int, color: str) -> str:
        # Return the ASCII art line with color markup applied to entire line
